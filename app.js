...
// -------------------------------
// UI helpers (amount display + steps)
// -------------------------------
function formatQty(value){
  const v = Number(value);
  if(!isFinite(v) || v <= 0) return '';
  // whole + common fractions for clean UI (keeps internal decimals)
  const fr = {
    0.25:'1/4', 0.33:'1/3', 0.5:'1/2', 0.66:'2/3', 0.75:'3/4'
  };
  const whole = Math.floor(v + 1e-9);
  const rem = +(v - whole).toFixed(2);
  const key = Object.keys(fr).map(Number).find(k => Math.abs(k - rem) < 0.02);
  let out = whole > 0 ? String(whole) : '';
  if(key != null){
    out += (out ? ' ' : '') + fr[key];
  } else if(rem > 0.001){
    // show 1 decimal max if not a common fraction
    const remNice = (Math.abs(rem - Math.round(rem)) < 1e-9) ? String(Math.round(rem)) : String(+rem.toFixed(1));
    out += (out ? ' ' : '') + remNice;
  }
  return out || String(+v.toFixed(1));
}
...
function unitStep(unit){
  ...
  if(u === 'lb') return 0.25;
  if(u === 'oz') return 1;
  if(u === 'cloves') return 1;
  if(u === 'eggs' || u === 'pieces' || u === 'slices' || u === 'medium') return 1;
  return 0.5;
}
...
function qtyStr(ing){
  ...
  const nice = formatQty(val) || String(+val.toFixed(1));
...
function titleFrom(ings){
  ...
  const hasLeafy = ings.some(i=>/(lettuce|spinach|arugula|mixed greens|kale)/i.test(i.name));
...
function render(){
  ...
  if(ul){
    ...
    state.ingredients.forEach((ing)=>{
      ...
      // Amount controls (adjust THIS ingredient only)
      const ctrl = document.createElement('div');
      ctrl.className = 'amount-controls';

      const decAmt = document.createElement('button');
      decAmt.type = 'button';
      decAmt.className = 'amt-btn';
      decAmt.setAttribute('aria-label', 'Decrease amount');
      decAmt.textContent = '−';

      const incAmt = document.createElement('button');
      incAmt.type = 'button';
      incAmt.className = 'amt-btn';
      incAmt.setAttribute('aria-label', 'Increase amount');
      incAmt.textContent = '+';
      ...
      function refreshReadout(){
        ...
        const txt = formatQty(v) || (v ? String(+v.toFixed(1)) : '');
        readout.textContent = txt && ing.base.u ? `${txt} ${ing.base.u}` : (txt || '');
      }

      const step = unitStep(ing.base.u);

      decAmt.onclick = ()=>{
        if(ing.name === 'skip it') return;
        ing.base.v = Math.max(0, +(ing.base.v - step).toFixed(3));
        if(ing.base.v === 0) ing.base.u = ing.base.u || '';
        setOwned();
        computeMacrosPerServing();
        refreshReadout();
        // update main line too
        const q2 = qtyStr(ing);
        main.textContent = q2 ? `${q2} ${pretty(ing.name)}` : `${pretty(ing.name)}`;
      };

      incAmt.onclick = ()=>{
        if(ing.name === 'skip it') return;
        ing.base.v = +(ing.base.v + step).toFixed(3);
        setOwned();
        computeMacrosPerServing();
        refreshReadout();
        const q2 = qtyStr(ing);
      ...
      refreshReadout();
      ctrl.append(decAmt, readout, incAmt);
