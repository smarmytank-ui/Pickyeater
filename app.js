...
const BASE_QTY = {
  ...
  dairy:{ v:0.5, u:'cups' },    // e.g., cheese
  bread:{ v:4, u:'pieces' }     // e.g., tortillas
};
...
,
  ...
  bread: [
    { name:'tortillas',       unitOverride:'pieces', baseOverride:4,  instrPatchKey:'combine' },
    { name:'bread',           unitOverride:'slices', baseOverride:2,  instrPatchKey:'combine' },
    { name:'pita',            unitOverride:'pieces', baseOverride:2,  instrPatchKey:'combine' },
    { name:'skip it',         unitOverride:'',      baseOverride:0,   instrPatchKey:'combine' }
  ]
};
...
function titleFrom(ings){
  ...
  // Light "food type" detection (no AI, deterministic)
  if(all.includes('taco') || all.includes('tortilla')){
    return `Simple ${main} Tacos`;
  }
  if(all.includes('pizza')){
    return `Simple ${main} Pizza`;
  }
  if(all.includes('bbq') || all.includes('barbecue')){
    return `Simple BBQ ${main}`;
  }
