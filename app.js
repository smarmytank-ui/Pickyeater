...
// Measurement intent controls how ingredient amounts step up/down.
// (Does NOT change your existing quantity baselines; it only controls the +/- behavior.)
const MEASURE_INTENT = {
  bulk:    { step: 0.25, max: 12 },   // cups / lbs-ish
  support: { step: 0.5,  max: 24 },   // tbsp-ish
  flavor:  { step: 0.25, max: 8  },   // tsp-ish
  finish:  { step: 0.25, max: 10 }    // tsp-ish
};
...
function normalizeUnit(u){
  ...
  if(x === 'cup' || x === 'cups') return 'cups';
  if(x === 'tablespoon' || x === 'tablespoons' || x === 'tbsp' || x === 'tbsps') return 'tbsp';
  if(x === 'teaspoon' || x === 'teaspoons' || x === 'tsp' || x === 'tsps') return 'tsp';
  if(x === 'piece' || x === 'pieces') return 'pieces';
  if(x === 'count' || x === 'counts') return 'count';
  return x;
}

function stepFor(ing){
  const u = normalizeUnit(ing?.base?.u);
  if(u === 'tsp') return 0.25;
  if(u === 'tbsp') return 0.5;
  if(u === 'cups') return 0.25;
  if(u === 'lb') return 0.25;
  if(u === 'oz') return 1;
  if(u === 'eggs' || u === 'cloves' || u === 'pieces' || u === 'count' || u === 'slices' || u === 'medium') return 1;
  // fallback by intent
  const rule = MEASURE_INTENT[intentForRole(ing?.role)];
  return rule?.step ?? 1;
}
...
function maxFor(ing){
  ...
  if(u === 'cups') return 12;
  if(u === 'lb') return 5;
  if(u === 'count' || u === 'pieces' || u === 'eggs') return 12;
  const rule = MEASURE_INTENT[intentForRole(ing?.role)];
  return rule?.max ?? 12;
}
...
function gramsFor(name, unit, qty){
  ...
  const u0 = normalizeUnit(unit);
  // treat "count" as "pieces" for gram math unless a name-specific mapping exists
  const u = (u0 === 'count') ? 'pieces' : u0;
...
function qtyStr(ing){
  ...
  // display "count 4" for bread-ish items
  if(u === 'count'){
    return `count ${Math.round(val)}`;
  }

  const nice = (Math.abs(val - Math.round(val)) < 1e-9)
    ? String(Math.round(val))
    : (Math.abs(val*2 - Math.round(val*2)) < 1e-9) ? (val).toFixed(1).replace(/\.0$/,'') : val.toFixed(2).replace(/0$/,'').replace(/\.$/,'');
  return `${nice} ${u}`;
}
...
decBtn.type = 'button';
decBtn.className = 'btn ghost small';
decBtn.textContent = '−';

const incBtn = document.createElement('button');
incBtn.type = 'button';
incBtn.className = 'btn ghost small';
incBtn.textContent = '+';

const step = stepFor(ing);
const maxV = maxFor(ing);
...
incBtn.onclick = () => {
  ing.base.v = Math.min(maxV, ing.base.v + step);
  computeMacrosPerServing();
  render();
};

ctrl.append(decBtn, incBtn);
...
function wireEvents(){
  ...
  if(gen && !gen.dataset.wired){
    ...
    gen.onclick = ()=>{
      ...
      if(!hasFat) ingredients.push({ id: uid(), name:'skip it', role:'fat', base:{ v:0, u:'' }, swapMeta:null });
      if(!hasAcid) ingredients.push({ id: uid(), name:'skip it', role:'acid', base:{ v:0, u:'' }, swapMeta:null });

      const hasSeasoning = ingredients.some(i=>i.role==='seasoning');
      if(!hasSeasoning) ingredients.push({ id: uid(), name:'skip it', role:'seasoning', base:{ v:0, u:'' }, swapMeta:null });
